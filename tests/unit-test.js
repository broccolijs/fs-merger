...
describe('fs-reader', function () {
  ...
  describe('Reads file from given location', function () {
    ...
    it('test-sub-1/test-sub-sub-1/sub-sub-c.txt', function () {
      let content = fsMerger.fs.readFileSync(
        'test-sub-1/test-sub-sub-1/sub-sub-c.txt',
        'utf-8'
      );
  ...
  describe('Reads contents of the folder from location', function () {
    let fsMerger = new FSMerge([
      'fixtures/test-1',
      'fixtures/test-2',
      new UnwatchedDir('fixtures/test-3'),
    ]);
    ...
    describe('reading folder invalid folder will throw', function () {
      ...
      it('readdir', function (done) {
        fsMerger.fs.readdir('sfsd', function (err) {
          expect(err.message).to.be.contains(
  ...
  describe('Verify few fs operations', function () {
    ...
    it('absolute path is accepted only with allowed operations', function () {
      ...
      expect(() => {
        ...
        `Operation rmdirSync is not allowed with FSMerger.fs. Allowed operations are readFileSync,existsSync,lstatSync,statSync,readdirSync,readdir,readFileMeta,entries,at`
      );
    });
    ...
    it('writeFileSync operation must throw error', function () {
      ...
      expect(() => {
        ...
        `Operation writeFileSync is not allowed with FSMerger.fs. Allowed operations are readFileSync,existsSync,lstatSync,statSync,readdirSync,readdir,readFileMeta,entries,at`
      );
    });
    ...
    it('acessing FSMerger non whitelisted operation must throw error', function () {
      ...
      expect(() => {
        ...
        `Operation _generateMap is not allowed with FSMerger.fs. Allowed operations are readFileSync,existsSync,lstatSync,statSync,readdirSync,readdir,readFileMeta,entries,at`
      );
    });
    ...
    it('acessing FSMerger non whitelisted properties must throw error', function () {
      ...
      expect(() => {
        ...
        `Operation _dirList is not allowed with FSMerger.fs. Allowed operations are readFileSync,existsSync,lstatSync,statSync,readdirSync,readdir,readFileMeta,entries,at`
      );
    });
  ...
  describe('Returns entries for', function () {
    ...
    it('root path', function () {
      ...
      let walkList = [
        ...
        'test-1/b.txt',
        'test-sub-1/',
        'test-sub-1/sub-b.txt',
        'test-sub-1/sub-c.txt',
        'test-sub-1/test-sub-sub-1/',
        'test-sub-1/test-sub-sub-1/sub-sub-b.txt',
        'test-sub-1/test-sub-sub-1/sub-sub-c.txt',
        'test-sub-2/',
        'x.txt',
      ];
    ...
    it('test-sub-1', function () {
      ...
      let walkList = [
        ...
        'sub-c.txt',
        'test-sub-sub-1/',
        'test-sub-sub-1/sub-sub-b.txt',
        'test-sub-sub-1/sub-sub-c.txt',
      ];
      fsEntries.forEach((entry) => {
        fileList.push(entry.relativePath);
      });
  ...
  describe('relativePathTo', function () {
    ...
    it('works', function () {
      let { relativePath, at } = fsMerger.relativePathTo(
        path.resolve('fixtures/test-1/a.txt')
      );
    ...
    it('requires an absolute path', function () {
      expect(() => {
        ...
        'relativePathTo expects an absolute path: fixtures/test-3/b.txt'
      );
    });
  });
});
